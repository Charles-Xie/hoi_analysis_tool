...
def plot_pr_curve_diff(pre1, rec1, pre2, rec2, score1, score2, fnames1, fnames2, verb_name_dict, demo_dir=None):
    for i in tqdm(range(len(verb_name_dict))):
        # plt.plot(rec1[i][1:], pre1[i][1:], label='QPIC PR', color="red")
        # plt.plot(rec2[i][1:], pre2[i][1:], label='QPIC+ours PR', color="blue")
        # if i == 10:
        #     import ipdb; ipdb.set_trace()
        s1 = np.concatenate(([0.], np.sort(score1[i])[::-1], [0.]))
        s2 = np.concatenate(([0.], np.sort(score2[i])[::-1], [0.]))
        r1_p, p1_p, s1_p = get_pr_10points(rec1[i][1:-1], pre1[i][1:-1], s1[1:-1])
        r2_p, p2_p, s2_p = get_pr_10points(rec2[i][1:-1], pre2[i][1:-1], s2[1:-1])
        plt.plot(r1_p, p1_p, label='QPIC PR', marker="o", markersize=4, color="red")
        plt.plot(r2_p, p2_p, label='QPIC+ours PR', marker="o", markersize=4, color="blue")
        plt.plot(r1_p, s1_p, label='QPIC SR', marker="o", markersize=4, color="red", linestyle="dashed")
        plt.plot(r2_p, s2_p, label='QPIC+ours SR', marker="o", markersize=4, color="blue", linestyle="dashed")
        # plt.plot(rec1[i][1:], s1[1:], label='QPIC SR', color="red", linestyle="dashed")
        # plt.plot(rec2[i][1:], s2[1:], label='QPIC+ours SR', color='blue', linestyle="dashed")
        ...
        plt.savefig(os.path.join(demo_dir, "{}_10points.pdf".format(i)), bbox_inches='tight')
        plt.clf()

def get_pr_10points(r, p, s):
    r_points = np.arange(0.0, 1.1, 0.1)
    p_list = []
    s_list = []
    for t in r_points:
        if np.sum(r >= t) == 0:
